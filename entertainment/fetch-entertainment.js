...
async function fetchEntertainmentMatrix() {
    ...
    try {
        ...
        const res = await fetchRSS('https://www.reddit.com/r/movies+television/.rss?limit=10');
    ...
    const musicSources = [
        ...
        { name: 'Consequence', url: 'https://consequence.net/feed/', logo: 'https://www.google.com/s2/favicons?domain=consequence.net&sz=128' },
    ];
    ...
    const ENT_REGION_LABELS = {
        'us':            'Google Trends Entertainment US',
        'north-america': 'Google Trends Entertainment North America',
        'south-america': 'Google Trends Entertainment South America',
        'europe':        'Google Trends Entertainment Europe',
        'africa':        'Google Trends Entertainment Africa',
        'asia':          'Google Trends Entertainment Asia',
        'oceania':       'Google Trends Entertainment Oceania'
    };

    const parseEntTrends = (xmlText, label) => {
        ...
        items.slice(0, 4).forEach(itemStr => {
            ...
            if (!storyContext) storyContext = `Trending entertainment search in ${ENT_REGION_LABELS[label]?.replace('Google Trends Entertainment ', '') || 'this region'}.`;
            ...
            results.push({
                site: trendName,
                sourceName: ENT_REGION_LABELS[label] || 'Google Trends Entertainment',
    ...
    const fetchEntContinentTrends = async (geos, label) => {
        const allItems = [];
        for (const geo of geos) {
            try {
                const res = await fetch(`https://trends.google.com/trending/rss?geo=${geo}&cat=3`, { headers: BROWSER_HEADERS });
                if (res.ok) allItems.push(...parseEntTrends(await res.text(), label));
            } catch(e) {}
        }
        const seen = new Set();
        const unique = allItems.filter(i => { if (seen.has(i.site)) return false; seen.add(i.site); return true; });
        entertainmentTrends.push(...unique.slice(0, 4));
    };

    try {
        console.log("Parsing US Entertainment Trends...");
        const usRes = await fetch('https://trends.google.com/trending/rss?geo=US&cat=3', { headers: BROWSER_HEADERS });
        console.log(`Google Trends Entertainment US status: ${usRes.status}`);
        if (usRes.ok) entertainmentTrends.push(...parseEntTrends(await usRes.text(), 'us'));
        console.log(`US entertainment trends compiled: ${entertainmentTrends.length} items`);
    } catch(e) { console.error('Entertainment Trends US Error:', e.message); }

    await fetchEntContinentTrends(['CA', 'MX'], 'north-america');
    await fetchEntContinentTrends(['BR', 'AR', 'CO', 'CL', 'PE'], 'south-america');
    await fetchEntContinentTrends(['GB', 'DE', 'FR', 'IT', 'ES', 'PL', 'NL'], 'europe');
    await fetchEntContinentTrends(['NG', 'ZA', 'KE', 'EG', 'GH', 'ET'], 'africa');
    await fetchEntContinentTrends(['IN', 'JP', 'KR', 'ID', 'PH', 'PK', 'VN', 'TH'], 'asia');
    await fetchEntContinentTrends(['AU', 'NZ'], 'oceania');
    console.log(`Entertainment trends total: ${entertainmentTrends.length} items`);
    ...
    if (entertainmentTrends.length === 0) {
        ...
        entertainmentTrends = Array.from({ length: 28 }, (_, i) => ({
            site: `Trending Entertainment Search #${i + 1}`,
            sourceName: ENT_REGION_LABELS[labels[Math.floor(i / 4)]] || 'Google Trends Entertainment',
            category: "Trending Entertainment Searches",
