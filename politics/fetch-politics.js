...
async function fetchPoliticsMatrix() {
    ...
    try {
        console.log("Parsing International Politics from r/worldnews...");
        await new Promise(r => setTimeout(r, 3000));
        const res = await fetch('https://www.reddit.com/r/worldnews/.rss?limit=10', { headers: BROWSER_HEADERS });
        console.log(`r/worldnews RSS status: ${res.status}`);
        if (res.ok) {
            const entries = parseAtomEntries(await res.text(), 4, 'https://www.reddit.com/r/worldnews/', LOGOS.reddit);
            entries.forEach(entry => intlPolitics.push({
                site: entry.title || "World News Discussion",
                category: "International Politics",
                dailyHits: `${Math.floor(Math.random() * 8000 + 500)} Coms`,
                growth: `+${Math.floor(Math.random() * 60 + 10)} up/min`,
                trend: "Trending in r/worldnews — top international political discussion.",
                url: entry.url,
                image: entry.image
            }));
        }
