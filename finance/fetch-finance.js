...
async function fetchFinanceMatrix() {
    ...
    try {
        ...
        const res = await fetchRSS('https://www.reddit.com/r/stocks+investing/.rss?limit=10');
        ...
        if (res.ok) {
            ...
            const subredditName_f = (url) => { const m = url.match(/\/r\/([^/+]+)/); return m ? `r/${m[1]}` : 'r/stocks'; };
            entries.forEach(entry => markets.push({
                site: entry.title || "Market Discussion",
                sourceName: subredditName_f(entry.url || ''),
                category: "Markets",
